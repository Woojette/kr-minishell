...
int	appliquer_inoutfile_ihoa(t_cmd *cmd, t_var_cmd *var_cmd)
{
  ...
  var_cmd->new_tab_char = add_double_tab_char(cmd[var_cmd->index_cmd].inoutfile, var_cmd->file_temp, var_cmd->size_file_tab);
  ...
  var_cmd->new_tab_int = add_double_tab_int(cmd[var_cmd->index_cmd].ihoa, 4, var_cmd->size_file_tab);
...
int	appliquer_add_cmd_heredoc_1(t_token *token, t_cmd *cmd, t_var_cmd *var_cmd)
{
  if (!token || !token->str || !cmd || !var_cmd || var_cmd->index_cmd < 0)
    return (-1);
	var_cmd->file_temp = ft_strdup("/dev/null"); // dupliquer le nom du fichier (redirection >) pour stocker dans cmd
	if (!var_cmd->file_temp)
		return (-1);
  if (appliquer_inoutfile_ihoa(cmd, var_cmd) == -1)
    return (-1);
	var_cmd->n = cmd[var_cmd->index_cmd].compter_heredoc; // n = nombre de heredoc deja existant pour cette commande
...
{
	var_cmd->new_tab_char = add_double_tab_char(cmd[var_cmd->index_cmd].limiter, var_cmd->limiter_sans_quote, var_cmd->n);
	// sauvegarder et agrandir le tableau limiter (sans quote) pour ajouter le nouveau limiter
	if (!var_cmd->new_tab_char)
		return (free(var_cmd->limiter_sans_quote), var_cmd->limiter_sans_quote = NULL, -1);
	cmd[var_cmd->index_cmd].limiter = var_cmd->new_tab_char; // mettre a jour le tableau de la structure avec le nouveau tableau agrandi
	var_cmd->new_tab_int = add_double_tab_int(cmd[var_cmd->index_cmd].hd_env, var_cmd->limiter_env, var_cmd->n);
	// sauvegarder le type de l'expansion de l'env de chaque cas
	if (!var_cmd->new_tab_int)
		return (-1);
	cmd[var_cmd->index_cmd].hd_env = var_cmd->new_tab_int; // mettre a jour le tableau de la structure avec le nouveau tableau agrandi
	cmd[var_cmd->index_cmd].compter_heredoc++; // incrementer le nombre de heredoc pour cette commande
	cmd[var_cmd->index_cmd].heredoc = 1; // marquer que c'est heredoc (<<)
