...
t_cmd	*malloc_cmd(t_token *token)
{
	...
	while (j < nbr_cmd) // j est index, donc ca commence par 0
	{
		...
		//////////////////// 유경 ihoa 로 통합할것 cmd[j].in_hd_index = NULL; // index de chaque infile(<) et limiter(<<) qui concerne chaque redir de in_heredoc
...
int add_cmd(t_token *token, t_cmd *cmd)
{
	...
	while (token) // pendant que le noeud dans la liste chainee existe
	{
		...
		else if (token->type_token == T_FD_HEREDOC)
		{
			...
			// new_tab_int = add_double_tab_int(cmd[index_cmd].ihoa, n, cmd[index_cmd].compter_ihoa);
			// // sauvegarde l'index de infile de cette heredoc redir 이 인덱스 안만들거임, compter 기준으로 숫자만 가질거임
			// if (!new_tab_int)
			// 	return (-1);
			// cmd[index_cmd].ihoa = new_tab_int; // mettre a jour le tableau de la structure avec le nouveau tableau agrandi
			// if (!cmd[index_cmd].ihoa)
